#include <bits/stdc++.h>
using namespace std;
void solve() {
  int n, m;
  double k;
  cin >> n >> m >> k;
  vector<array<int, 4>> a(n + 10);
  for (int i = 1; i <= n; i++) {
    int v, p, h;
    cin >> v >> p >> h;
    a[i] = {v, -h, -p, i};
  }
  vector<pair<double, int>> f(m + 10);
  priority_queue<tuple<double, int, int>> f2;
  for (int i = 1; i <= m; i++) {
    int p, h;
    cin >> p >> h;
    f[i] = {h * 1.0, p};
    f2.push({h * 1.0, p, i});
  }
  sort(a.begin() + 1, a.begin() + n + 1, greater());

  vector<int> ans(n + 10);
  for (int we = 1; we <= n; we++) {
    auto [v, h, p, i] = a[we];
    p = -p, h = -h;
    // cout << "I: " << i << " v: " << v << " p: " << p << " h: " << h << "\n";
    vector<tuple<int, double, int>> cun;
    for (int w = 1; w <= m; w++) {
      auto [H1, P1, j1] = f2.top();
      H1 = f[j1].first;
      f2.pop();
      f2.push({H1, P1, j1});
      auto [H, P, j] = f2.top();
      H = f[j].first;
      f2.pop();
      // cout << "J: " << j << "P: " << P << " H: " << H << '\n';
      if (P >= p && H >= h) {
        if (j != 1) {
          if (f[j - 1].first < k * double(h)) {
            cun.push_back({H, P, j});
            continue;
          }
        }
        if (j != m) {
          if (f[j + 1].first < k * double(h)) {
            cun.push_back({H, P, j});
            continue;
          }
        }
        f[j].second -= p;
        f[j].first -= h;
        if (j != 1)
          f[j - 1].first -= k * h;
        if (j != n)
          f[j + 1].first -= k * h;
        ans[i] = j;
        cun.push_back({H - h, P - p, j});
        break;
      } else {
        cun.push_back({H, P, j});
      }
    }
    for (int kk = 0; kk < cun.size(); kk++) {
      auto [H, P, j] = cun[kk];
      // cout << "     J: " << j << "P: " << P << " H: " << H << " fs"
      //      << f[j].second << '\n';
      f2.push({f[j].first, P, j});
    }
  }
  for (int i = 1; i <= n; i++)
    cout << ans[i] << " ";
}
int main() {
  ios::sync_with_stdio(0);
  cin.tie();
  solve();
}